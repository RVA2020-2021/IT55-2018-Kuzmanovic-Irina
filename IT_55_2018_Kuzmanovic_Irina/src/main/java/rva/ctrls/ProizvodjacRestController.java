...
public class ProizvodjacRestController {
	...
	@ApiOperation(value="Vraca proizvodjaca iz baze podataka ciji je id vrednost prosledjena kao path varijabla")
	...
	@ApiOperation(value="Vraca kolekciju svih proizvodjaca iz baze podataka koji u nazivu sadrze string prosledjen kao path varijabla")
	...
	@ApiOperation(value="Modifikuje postojeceg proizvodjaca u bazu podataka")
	...
	@ApiOperation(value="Brise proizvodjaca iz baze podataka ciji je id vrednost prosledjena kao path varijabla")
	public ResponseEntity<Proizvodjac> deleteproizvodjac(@PathVariable("id") Integer id){
		...
		if(id == -100) {
			jdbcTemplate.execute(
					...
					+ "VALUES (-100, 'Proizvodjac test','Adresa test','+325489987')"
					);
		}
		return new ResponseEntity<Proizvodjac>(HttpStatus.OK);
	}
